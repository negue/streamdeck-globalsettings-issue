...
switch (os.platform()) {
	case "darwin":
		child_process.execSync(
			`cp -R build/com.acme.myStreamdeckPlugin.sdPlugin ${currentPlatform.pluginsFolderPath}/`
		);
		break;
	case "win32":
		fs.copySync(
			"build/com.acme.myStreamdeckPlugin.sdPlugin",
			`${currentPlatform.pluginsFolderPath}\\com.acme.myStreamdeckPlugin.sdPlugin`
		);
		break;
